...
class RAGSystemBGE:
        ...
        try:
            ...
            if use_server:
                ...
                try:
                    ...
                    # 기존 'bge' 컬렉션의 ColBERT HNSW 활성화 (m=0 → m=16)
    ...
    def _search_documents_qdrant(
        ...
        try:
            ...
            # -----------------------------
            # 1) 후보군 예산(문서 4,000개 기준 권장값)
            # -----------------------------
            B = max(200, min(400, top_k * 30))  # top_k=10 -> 300
            dense_limit = int(B * 0.45)
            sparse_limit = B - dense_limit

            # =========================================================
            # DEBUG ROUTINE: dense-only / sparse-only / hybrid-RRF 비교
            # =========================================================
            if debug:
                ...
                # hybrid-RRF
                h_res = self.qdrant_client.query_points(
                    collection_name=collection_name,
                    prefetch=[
                    ...
                    limit=min(50, B), with_payload=True)
                h_pts = getattr(h_res, "points", h_res)
                ...
                _top_print("hybrid-RRF", h_pts, n=10)

                # 교집합/차집합 요약
                D, S, H = _to_fp_set(d_pts), _to_fp_set(s_pts), _to_fp_set(h_pts)
                print(
                    f"\n[DEBUG sets]"
                    f" |D|={len(D)} |S|={len(S)} |H|={len(H)}"
                    f" |H∩D|={len(H & D)} |H∩S|={len(H & S)}"
                    f" |H-D|={len(H - D)} |H-S|={len(H - S)}"
                )

            # -----------------------------
            # 3) Hybrid-RRF로 최종 top_k 반환 (2-way: dense + sparse)
            # -----------------------------
            ...
            print(f"✅ Qdrant 검색 완료: {len(results)}개 문서 반환 (B={B}, dense={dense_limit}, sparse={sparse_limit})")
            return results
